...
export default async function WorkerAvailabilityPage(){
 ...
 return <main className="min-h-screen overflow-x-hidden bg-[#f3efe6] px-3 py-4 text-[#12211a] sm:px-5 sm:py-6 lg:px-8 lg:py-8">
  <div className="mx-auto w-full max-w-6xl min-w-0">
   ...
   <section className="mt-4 grid min-w-0 gap-4 sm:mt-5 lg:grid-cols-2">
    {products.length?products.map(product=><article key={product.id} className="min-w-0 overflow-hidden rounded-[22px] border border-black/10 bg-[#fffdf8] p-5 sm:rounded-[24px] sm:p-6">
     ...
     <div className="mt-4 flex min-w-0 items-start gap-2 text-sm font-bold leading-5 text-[#1d513b]"><Sparkles className="mt-0.5 shrink-0" size={16}/><span className="min-w-0 break-words">{product.readiness_simulation_id?"Matching readiness simulation":"Preparation guidance available"}</span></div>
